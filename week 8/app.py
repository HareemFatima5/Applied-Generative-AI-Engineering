...
def render_hero():
    ...
    st.markdown(
        ...
        <p class="hero-sub">Upload text, PDF, or spreadsheet files (CSV, TSV, XLSX), then ask a question and get an answer grounded in what they actually say.</p>
        """,
        ...
        if os.path.isdir(DOCUMENTS_DIR):
            ...
            if files:
                for f in files:
                    ...
                    is_spreadsheet = os.path.splitext(f)[1].lower() in (".xlsx", ".xls")
                    unit = "sheet" if is_spreadsheet else "page"
                    if page_count:
                        label = f"{f} ({page_count} {unit}{'s' if page_count != 1 else ''})"
                    else:
                        label = f
...
def render_upload_section():
    ...
    uploaded_files = st.file_uploader(
        ...
        type=["txt", "md", "pdf", "csv", "tsv", "xlsx", "xls"],
