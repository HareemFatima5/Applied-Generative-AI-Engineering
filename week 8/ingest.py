...
SUPPORTED_EXTENSIONS = {".txt", ".md", ".pdf", ".csv", ".tsv", ".xlsx", ".xls"}
TABULAR_EXTENSIONS = {".csv", ".tsv", ".xlsx", ".xls"}

TABLE_ROWS_PER_CHUNK = 40
TABLE_MAX_CHARS_PER_CHUNK = 3000
...
def load_documents(documents_dir: str = DOCUMENTS_DIR) -> list[dict]:
    ...
    for path in sorted(Path(documents_dir).glob("*")):
        ...
        if suffix in (".txt", ".md"):
            ...
            pages = _extract_csv_pages(path)
            metadata = {"title": None, "author": None, "subject": None,
                        "creation_date": None, "creation_date_iso": None,
                        "page_count": None, "title_guessed": False}
        elif suffix in (".xlsx", ".xls"):
            pages = _extract_excel_pages(path)
            metadata = _extract_excel_metadata(path, page_count=len(pages) or None)
        else:
            continue
...
def _dataframe_to_table_chunks(df, sheet_name: str | None = None) -> list[str]:
    """Turn a dataframe into row-batched text chunks, each self-contained with
    the header line (and sheet name, if any) so a single retrieved chunk is
    still interpretable without the rest of the table."""
    if df.empty:
        return []

    header_line = " | ".join(str(c) for c in df.columns)
    prefix = f"Sheet: {sheet_name}\n{header_line}\n" if sheet_name else f"{header_line}\n"
    ...
    for line in row_lines:
        # whichever limit is hit first - row count or character budget - triggers
        # a new chunk, so wide tables with few rows still stay embedding-sized
        if current_rows and (
            len(current_rows) >= TABLE_ROWS_PER_CHUNK
            or current_len + len(line) > TABLE_MAX_CHARS_PER_CHUNK
        ):
            flush()
            current_rows = []
            current_len = len(prefix)
...
def _extract_csv_pages(path: Path) -> list[tuple[int | None, str, list[str]]]:
    import pandas as pd
    ...
    try:
        if path.suffix.lower() == ".tsv":
            ...
            # sniff the delimiter (comma, semicolon, pipe, ...) rather than assuming comma
            df = pd.read_csv(path, sep=None, engine="python", **read_kwargs)
    except Exception as e:
        print(f"  [warn] could not read '{path.name}' as tabular data: {e}")
        return []

    chunks = _dataframe_to_table_chunks(df)
    if not chunks:
        return []
    # no natural page number for a flat csv/tsv file, mirrors how .txt/.md are handled
    return [(None, "", chunks)]


def _extract_excel_pages(path: Path) -> list[tuple[int | None, str, list[str]]]:
    import pandas as pd
    ...
    for i, (sheet_name, df) in enumerate(sheets.items(), start=1):
        chunks = _dataframe_to_table_chunks(df, sheet_name=sheet_name)
        if chunks:
            # sheet number stands in for "page" so it flows through the existing
            # page-display logic in the retriever/UI unchanged
            pages.append((i, "", chunks))
    return pages


def _extract_excel_metadata(path: Path, page_count: int | None) -> dict:
    metadata = {"title": None, "author": None, "subject": None,
                "creation_date": None, "creation_date_iso": None,
                "page_count": page_count, "title_guessed": False}
    try:
        import openpyxl
        ...
        if props.created:
            ...
            metadata["creation_date_iso"] = props.created.date().isoformat()
        wb.close()
    except Exception as e:
        print(f"  [warn] could not read workbook metadata from '{path.name}': {e}")
    return metadata
