...
from ingest import load_documents, build_chunks, build_vector_store, DOCUMENTS_DIR, VECTOR_STORE_DIR
    ...
    for chunk in data["chunks"]:
        ...
        if source not in by_source:
            by_source[source] = {
                ...
                "title_guessed": chunk.get("title_guessed", False),
                "author_guessed": chunk.get("author_guessed", False),
            }
    return by_source
            ...
            if files:
                for f in files:
                    ...
                    with st.expander(label, expanded=False):
                        ...
                        if meta.get("author"):
                            author_label = "Author" if meta.get("author_guessed") else "Author"
                            st.caption(f"{author_label}: {meta['author']}")
                        if meta.get("subject"):
...
def save_uploaded_files(uploaded_files, replace_existing: bool) -> int:
    """Write uploaded files into the documents/ folder and return how many were saved."""
...
def build_index_in_app(progress_callback=None) -> int:
    """Run the same pipeline as ingest.py, but in-process so the app stays interactive."""
