...
def build_context(chunks: list[dict]) -> str:
    # numbers each chunk and tags it with source/page/title/date so the model can attribute claims back to a specific passage
    parts = []
    for i, chunk in enumerate(chunks, start=1):
        ...
        if title:
            location = f"{location}, title: {title}"
        creation_date = chunk.get("creation_date_iso")
        if creation_date:
            location = f"{location}, dated: {creation_date}"
...
def generate_with_gemini(question: str, context: str, api_key: str) -> str:
    ...
    # low temperature to keep answers grounded/deterministic rather than creative
...
def generate_fallback(chunks: list[dict]) -> str:
    # used when gemini isn't available or its call fails
    if not chunks:
...
def answer_question(question: str, top_k: int = DEFAULT_TOP_K, retriever: Retriever = None) -> dict:
    ...
    if GEMINI_AVAILABLE and api_key:
        try:
            ...
            # gemini failure will not kill the whole request degrade to raw passage instead
...
def main() -> None:
    ...
    # allow the question as either CLI args or if omitted, an interactive prompt
